...
### 一句話核心
MCP 不是取代 API，而是**架在 API 之上**的語義層——把靜態的 API 端點，變成模型能自行推理、發現、呼叫的「活介面」。

### API 為什麼餵不飽 LLM
- API 是**程式對程式**的確定性契約：定義端點 → 送 request → 拿 response，清楚可預測，傳統軟體很完美。
- 但 LLM 不是只呼叫單一端點：它要串接多個工具、解讀非結構化資料、追問下一步——它需要的是 **context（脈絡）**，不只是工具存取權。
- API 像一個上鎖的櫃子：你得知道開哪個抽屜、鑰匙什麼形狀。模型卻在沒有標籤的情況下猜櫃子裡有什麼，只能靠人**硬編碼 + 反覆 prompt 說明**。

### MCP 翻轉了什麼假設
- API 假設「雙方都事先知道會拿到什麼」；MCP 反過來，給模型一份**機器可讀的即時地圖**，讓它動態發現工具能做什麼、要什麼輸入、回什麼輸出。
- 差別本質：API 是兩個應用之間的**程式碼層契約**；MCP 是模型與環境之間的**語義協定**。
- 「呼叫哪個工具、何時呼叫」的邏輯，從你的 app code **移進模型的推理層**。等於給模型一個工具箱，而不是逼它背每個工具怎麼用。

### 底層怎麼運作
- MCP server 是一個輕量程序，貼著你的服務／資料源運行，用 **JSON schema** 描述自己能做什麼。
- 模型透過標準介面（WebSocket / HTTP）連上，取得 metadata → 直接呼叫函式，不用猜。一切**自描述**，不必再 prompt engineering schema 或改寫回應格式。
- 效益：不再蓋 100 個客製整合，只蓋**一個 MCP 介面**，所有相容模型即插即用。

### MCP on top of API（關鍵定位）
...
- **安全與控制**：模型能直接呼叫工具，就得有清楚的權限層（避免誤刪檔、亂寄信、亂改資料庫）。規格已定義 capabilities、scopes、認證方法，但仍屬早期。
- **開發者心態**：從「端點與路由」思維，轉向「能力與脈絡」思維——描述系統「能做什麼」而非「怎麼做」。

### 大局類比
如同 HTTP 統一了 FTP／Gopher／Telnet 各自為政的網路協定，MCP 想為 AI agent 做同樣的事：連接器蓋一次，任何相容模型（Gemini、Claude、GPT）都能立刻使用。未來 AI 工具會從「客製整合」走向「共享生態」，讓**模型成為軟體的一等公民使用者**。
